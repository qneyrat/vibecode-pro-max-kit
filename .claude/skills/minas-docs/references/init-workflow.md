...
3. Activate `minas:scout` skill to explore the code base and return detailed summary reports to the main agent
...
- `.minas/process/context/all-context.md`: Root context entrypoint, routing table, broad repository context, architecture, conventions, and current state.
- `.minas/process/context/tests/all-tests.md`: Testing contract and index to `.minas/process/context/tests/`.
- Existing `.minas/process/context/*/all-*.md` files: Group-level quick entrypoint for any durable context group that already exists.
- Topic-specific `.minas/process/context/*.md`: Durable current-state knowledge selected by the router.
...
Run `node .claude/skills/minas-audit-context/scripts/validate-context-discovery.mjs` and report
