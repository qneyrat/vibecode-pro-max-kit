...
`.minas/process/context/` is the durable shared project-knowledge layer for agents. It is not the same as feature planning or tool-specific memory.
...
- Update `.minas/process/context/all-context.md` whenever a new durable entrypoint is added, renamed, grouped, or removed.
- Move or split one group at a time so discovery changes stay reviewable.
- After context-organization changes, run the `minas-audit-context` skill or its validators.
...
- Durable shared knowledge that both systems should rely on belongs in `.minas/process/context/`.
