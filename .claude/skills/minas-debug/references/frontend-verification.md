...
Visual verification of frontend implementations using Chrome MCP (Claude Chrome Extension) or `minas:chrome-devtools` skill fallback.
...
Check if Chrome MCP server is available via `minas:mcp-management` skill or `ListMcpResourcesTool`:
...
When Chrome MCP is not configured, use `minas:chrome-devtools` skill (Puppeteer with bundled Chromium):
...
If `minas:chrome-devtools` skill is also unavailable, skip visual verification and note in report:
