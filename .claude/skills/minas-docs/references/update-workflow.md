...
3. Activate `minas:scout` skill to explore the code base and return detailed summary reports
...
1. Read `.minas/process/context/all-context.md` first.
2. Count docs: `find .minas/process/context -name '*.md' | sort | wc -l`
3. Get LOC: `find .minas/process/context -name '*.md' -print0 | xargs -0 wc -l | sort -rn`
4. Strategy:
...
- `.minas/process/context/all-context.md`: Update routing, indexes, group lifecycle, read-when rules, and broad repo context when architecture or conventions changed.
- `.minas/process/context/tests/all-tests.md`: Update the testing contract and links to `.minas/process/context/tests/`.
- Existing `.minas/process/context/*/all-*.md` files: Update group indexes, quick procedures, and ownership rules when grouped docs change.
- Topic-specific root or grouped context docs under `.minas/process/context/`: Update only when selected by the router.
...
1. Run `find .minas/process/context -name '*.md' -print0 | xargs -0 wc -l | sort -rn` to check LOC
...
1. Run: `node .claude/skills/minas-audit-context/scripts/validate-context-discovery.mjs`
