...
Use the `Reports`/`Plans` path from the orchestrator handoff when provided. Otherwise default to `.minas/process/general-plans/reports/`, or `.minas/process/features/{feature}/reports/` when a `Feature:` is named. Name reports `ui-ux-designer-{slug}.md`; plans follow `[feature]_PLAN_[dd-mm-yy].md`.
