...
1. Read `.minas/process/context/all-context.md` for the context routing protocol.
...
7. Regenerate or check the machine-readable skill catalog:
   ```bash
   node .claude/skills/minas-audit-context/scripts/generate-skills-catalog.mjs --write
...
- Treat `.claude/skills/minas-audit-context/references/skill-routing-policy.json` as the explicit allowlist for intentionally non-routed shared skills.
- Do not move large context files without updating `.minas/process/context/all-context.md`.
...
- Treat .minas/process/context/generated-skills-catalog.json as the machine-readable catalog owned by `audit-context`.
