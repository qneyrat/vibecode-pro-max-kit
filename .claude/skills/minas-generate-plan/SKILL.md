...
This skill is the canonical planning contract for the repo. Planning discipline previously spread across `minas:plan` now belongs here plus the `plan-agent` prompt.
...
phase-plan set under one feature folder. See `.minas/process/development-protocols/phase-programs.md`.
...
4. Save the plan to `.minas/process/general-plans/active/` unless the work belongs to an existing `.minas/process/features/{feature}/active/` folder.
5. Read `.minas/process/context/all-context.md` when present to choose relevant context docs.
6. For complex plans, read `.minas/process/development-protocols/references/example-complex-prd.md` before writing.
7. Include automated and manual verification gates from `.minas/process/context/tests/all-tests.md`.
...
- Prefer `.minas/process/features/{feature}/active/` when the topic maps to an existing feature folder.
